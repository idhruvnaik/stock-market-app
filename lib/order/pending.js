...
const placeOrderLib = async (unique_token, params) => {
  ...
  try {
    const { symbol, symbol_token, state, quantity, price, mode, expiry, raw } =
      params;
...
const listLib = async (unique_token, params) => {
  ...
  await authorize();
  const tokens = orders?.map((item) => item?.symbol_token);
  ...
  result = await Promise.all(
    orders?.map(async (order) => {
      const ltpData = await lastTradedPrices.find(
        (price) => price?.symbolToken === order?.symbol_token
      );
